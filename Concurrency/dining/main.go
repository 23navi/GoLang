...
func dine() {
	...
	for i := 0; i < len(philosophers); i++ {
		...
		go diningProblem(philosophers[i], wg, forks, seated, eatingEndMutex)
	}

	// Wait for the philosophers to finish. This blocks until the wait group is 0.
	wg.Wait()
	for i := 0; i < len(eatingEndOrder); i++ {
		fmt.Printf("%s ate\n", eatingEndOrder[i])
	}
}
...
func diningProblem(philosopher Philosopher, wg *sync.WaitGroup, forks map[int]*sync.Mutex, seated *sync.WaitGroup, eatingEndMutex *sync.Mutex) {
	defer wg.Done()
	...
	eatingEndMutex.Lock()
	eatingEndOrder = append(eatingEndOrder, philosopher.name)
	eatingEndMutex.Unlock()
